...
# def base(request):
#     return  render(request,'buyyer/base.html',locals())
@cookieValid
def index(request):
    data = []
...
def nameValid(request):
    if request.method =="GET" and request.GET:
        ...
        if user:
            return JsonResponse({"result":"no you can't"})
        else:
            return JsonResponse({"result":"yes you can"})
def logout(request):
...
def changeAddress(request,address_id):#修改
